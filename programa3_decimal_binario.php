<?php
// Programa 3: Conversión de decimal a binario
$numeroDecimal = '';
$arregloBinario = [];
$pasos = [];
$error = '';
...
if ($_POST['accion'] ?? '' === 'convertir') {
    ...
    if (!is_numeric($numeroDecimal) || $numeroDecimal < 0 || $numeroDecimal != (int)$numeroDecimal) {
        $error = 'Por favor, ingresa un número decimal válido (entero mayor o igual a 0)';
    } else {
        ...
        if ($numeroDecimal === 0) {
            $arregloBinario = [0];
            $pasos[] = 'El número 0 en binario es 0';
        } else {
...
<head>
    ...
    <title>Programa 3 - Conversión Decimal a Binario (PHP)</title>
    <style>
        body {
...
<body>
    <div class="container">
        <h1>Programa 3: Conversión de Decimal a Binario (PHP)</h1>
        ...
        <div class="input-section">
            <h3>Ingresa un número decimal:</h3>
            <form method="post">
        ...
        <?php if ($mostrarResultados && empty($error)): ?>
            ...
            <div class="resultado binario">
                <h3>Número Binario:</h3>
                <div>
                    <?php foreach ($arregloBinario as $index => $digito): ?>
                        <span class="digito" title="Posición <?php echo $index; ?>: <?php echo $digito; ?>">
                            <?php echo $digito; ?>
                        </span>
                    <?php endforeach; ?>
                </div>
            ...
            <div class="arreglo-info">
                <h3>Arreglo de Dígitos Binarios:</h3>
                <p><strong>Arreglo PHP:</strong> [<?php echo implode(', ', $arregloBinario); ?>]</p>
                <p><strong>Longitud:</strong> <?php echo count($arregloBinario); ?> dígitos</p>
                <p><strong>Número binario como string:</strong> <?php echo implode('', $arregloBinario); ?></p>
                <p><strong>Verificación con decbin():</strong> <?php echo decbin($numeroDecimal); ?></p>
                <p><strong>Verificación correcta:</strong> <?php echo (implode('', $arregloBinario) === decbin($numeroDecimal)) ? '✅ Sí' : '❌ No'; ?></p>
            </div>
            ...
            <div class="resultado proceso">
                ...
                <div class="paso">
                    <strong>Resultado:</strong> Los residuos leídos de abajo hacia arriba forman el número binario: <?php echo implode('', $arregloBinario); ?>
                </div>
                
                <div class="paso" style="background-color: #e8f5e8;">
                    <strong>Análisis PHP:</strong><br>
                    • Número decimal original: <?php echo $numeroDecimal; ?><br>
                    • Número binario calculado: <?php echo implode('', $arregloBinario); ?><br>
                    • Verificación con función nativa: <?php echo decbin($numeroDecimal); ?><br>
                    • Total de divisiones realizadas: <?php echo count($pasos); ?><br>
                    • Conversión de vuelta a decimal: <?php echo bindec(implode('', $arregloBinario)); ?>
                </div>
            </div>
        <?php endif; ?>
    </div>
